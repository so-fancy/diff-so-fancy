...
sub git_ansi_color {
	...
	if (grep { /^no-?bold$/ } @parts) {
		push(@ansi_part, "22");  # not a typo: no-bold and no-dim are the same.
	}
	...
	if (grep { /^no-?dim$/ } @parts) {
		push(@ansi_part, "22");
	}

	if (grep { /^italic$/ } @parts) {
		push(@ansi_part, "3");
	}

	if (grep { /^no-?italic$/ } @parts) {
		push(@ansi_part, "23");
	}
	...
	if (grep { /^no-?reverse$/ } @parts) {
		push(@ansi_part, "27");
	}

	if (grep { /^strike$/ } @parts) {
		push(@ansi_part, "9");
	}

	if (grep { /^no-?strike$/ } @parts) {
		push(@ansi_part, "29");
	}
...
sub color {
	...
	my %cmd_map = qw(bold 1 dim 2 italic 3 ul 4 blink 5 reverse 7 strike 9
					 nobold 22 nodim 22 noitalic 23 noul 24 noblink 25 noreverse 27 nostrike 29);
